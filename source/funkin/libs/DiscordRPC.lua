local function getLibPath(lib)
	return Flora.config.sourceFolder .. "/funkin/libs/" .. lib
end

local ffi, discordRPClib = require "ffi"
local OS = love.system.getOS()
if OS == "Windows" then
	if love.filesystem.getInfo(getLibPath("windows/discord-rpc.dll"), "file") then
		discordRPClib = getLibPath("windows/discord-rpc")
	else
		discordRPClib = "discord-rpc"
	end
elseif OS == "Linux" then
	if love.filesystem.getInfo(getLibPath("linux/libdiscord-rpc.so"), "file") then
		discordRPClib = getLibPath("linux/libdiscord-rpc")
	else
		discordRPClib = "libdiscord-rpc"
	end
elseif OS == "OS X" then
	if love.filesystem.getInfo(getLibPath("osx/libdiscord-rpc.dylib"), "file") then
		discordRPClib = getLibPath("osx/libdiscord-rpc")
	else
		discordRPClib = "libdiscord-rpc"
	end
end
